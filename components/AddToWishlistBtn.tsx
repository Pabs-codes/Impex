...
const AddToWishlistBtn = ({ product, slug }: AddToWishlistBtnProps) => {
  ...
  const addToWishlistFun = async () => {
    ...
    .catch(() => {
      toast.error("Error while adding product to the wishlist");
    });
    } else {
  ...
  const removeFromWishlistFun = async () => {
    if (session?.user?.email) {
      ...
      axiosInstance.get(`/users/email/${session?.user?.email}`)
      .then((response) => {
        return axiosInstance.delete(`/wishlist/${response.data.id}/${product?.id}`);
      })
      .then(() => {
        removeFromWishlist(product?.id);
        toast.success("Product removed from the wishlist");
      ...
      .catch(() => {
        toast.error("Error while removing product from the wishlist");
      });
    }
  };
  ...
  const isInWishlist = async () => {
    ...
    if (session?.user?.email) {
      ...
      axiosInstance.get(`/users/email/${session?.user?.email}`)
      .then((response) => {
        return axiosInstance.get(`/wishlist/${response.data.id}/${product?.id}`);
      })
      .then((response) => {
