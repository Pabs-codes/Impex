...
const Products = async ({ slug }: any) => {
  ...
  const subCategories = slug?.searchParams?.subCategories ? slug?.searchParams?.subCategories : [];
 ...
 // If out of stock checkbox is checked, stockMode is "lt"
  ...
  const products = await axiosInstance.get(`/products?filters[price][$lte]=${
    slug?.searchParams?.price || 3000
  }&filters[rating][$gte]=${
    Number(slug?.searchParams?.rating) || 0
  }&filters[inStock][$${stockMode}]=1&${
    slug?.params?.slug?.length > 0
      ...
      slug?.searchParams?.subCategories
          ? `filters[subCategories][$equals]=${subCategories}&`
          : ""
  }sort=${slug?.searchParams?.sort}&page=${page}`)
  .then((response) => {
