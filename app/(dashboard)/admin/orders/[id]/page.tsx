...
const AdminSingleOrder = () => {
  ...
  useEffect(() => {
    const fetchOrderData = async () => {
      ...
      // setOrder(data);
      axiosInstance.get(`/orders/${params?.id}`)
      .then((res) => {
        setOrder(res.data);
      })
      .catch((err) => {
    ...
    const fetchOrderProducts = async () => {
      ...
      axiosInstance.get(`/order-product/${params?.id}`)
      .then((res) => {
  ...
  const updateOrder = async () => {
    if (
      ...
      //   );
      axiosInstance.put(`/orders/${order?.id}`, order)
      .then(() => {
        toast.success("Order updated successfuly");
      })
      .catch(() => {
  ...
  const deleteOrder = async () => {
    ...
    //   });
    // });
    axiosInstance.delete(`/order-product/${order?.id}`)
    .then(() => {
